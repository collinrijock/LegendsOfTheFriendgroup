...
class GameRoom extends colyseus_1.Room {
    constructor() {
        ...
        this.battleTickInterval = null; // New name: For server-side battle timer countdown and attack ticks
        ...
        this.onMessage("playerUnready", (client) => {
            ...
            if (player && player.isReady === true) {
                ...
                if (this.state.currentPhase === GameState_1.Phase.Lobby ||
                    this.state.currentPhase === GameState_1.Phase.Shop ||
                    this.state.currentPhase === GameState_1.Phase.Preparation) {
                    console.log(`Player ${player.username} (${client.sessionId}) is un-readying for phase: ${this.state.currentPhase}`);
                    ...
                    console.warn(`Player ${player.username} (${client.sessionId}) attempted to unready during inappropriate phase: ${this.state.currentPhase}`);
                }
            }
        });
    ...
    startGenericPhaseTimer() {
        ...
        this.genericPhaseTimerInterval = this.clock.setInterval(() => {
            ...
            if (this.state.currentPhase === GameState_1.Phase.Shop || this.state.currentPhase === GameState_1.Phase.Preparation) {
    ...
    transitionToPhase(newPhase) {
        ...
        if (newPhase === GameState_1.Phase.Shop && this.state.currentDay === 1 && oldPhase === GameState_1.Phase.Lobby && !this.matchTimerInterval) {
        ...
        else if (newPhase === GameState_1.Phase.Preparation) {
            this.state.phaseTimer = PREPARATION_DURATION;
            this.startGenericPhaseTimer();
        }
        else if (newPhase === GameState_1.Phase.Battle) {
        ...
        if (newPhase === GameState_1.Phase.GameOver) {
            if (this.battleTickInterval)
                this.battleTickInterval.clear(); // Should be cleared by endBattle already
    ...
    startBattle() {
        ...
        this.battleTickInterval = this.clock.setInterval(() => {
            if (this.state.currentPhase === GameState_1.Phase.Battle) {
                ...
                const damageToApplyToTargets = new Map();
                ...
                damageToApplyToTargets.forEach((damageInfo, targetInstanceId) => {
                    var _a;
                    let targetCardSchema;
                    let targetPlayerIdForEvent;
                    ...
                    if (targetCardSchema && targetPlayerIdForEvent) {
                        ...
                        if (oldHp > 0) {
                            ...
                            console.log(`[SERVER BATTLE] ${targetCardSchema.name} (P: ${(_a = this.state.players.get(targetPlayerIdForEvent)) === null || _a === void 0 ? void 0 : _a.username}) takes ${damageInfo.totalDamage} total damage. HP: ${oldHp} -> ${targetCardSchema.currentHp}`);
                ...
                if (playerIds.length === 2) {
                    ...
                    if (player1 && player2) {
                        ...
                        if (!player1HasLivingCards || !player2HasLivingCards) {
                            console.log(`Battle timer loop: Potential board clear condition met. P1 HasLiving: ${player1HasLivingCards}, P2 HasLiving: ${player2HasLivingCards}. Calling endBattle.`);
                ...
                if (this.state.phaseTimer <= 0) {
                    console.log("Battle phase timer loop: Timer reached zero. Calling endBattle."); // Log before call
    ...
    endBattle(timeoutReached = false) {
        ...
        if (this.battleTickInterval)
            this.battleTickInterval.clear(); // Stop the timer
